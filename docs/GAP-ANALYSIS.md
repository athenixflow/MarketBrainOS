...
| §7, §26–29 | Token economy | 🟡 | Per-tool costs (3/4/5/6) kept deliberately — revisit later. **Fixed:** Pro = 200 tokens now consistent everywhere (was 50 in some copy). Free = 4 tokens (PRD says 5 — deferred). Top-up $5=100 ✅. |
| §8, §30–32 | Subscription & billing | ✅ (simulated) | Full lifecycle: `subscription_status` (free/active/past_due/cancelled/expired) + `plan_renews_at` on the profile; server-authoritative `changeSubscription` callable ([functions/src/index.ts](../functions/src/index.ts)) handles upgrade/cancel/renew/downgrade, grants 200 tokens, writes `payments` + action log. In-app [components/SubscriptionPanel.tsx](../components/SubscriptionPanel.tsx) (status badge, renewal date, manage actions) replaces all external "Upgrade" redirects; billing history via existing `PaymentHistoryModal`. **Real Stripe + Past Due/Expired automation = future deploy-time work** (the verification seam is marked in the callable). |
...
| §63–68 | Automation engine | 🟡 | Top-up + analysis-completion handled server-side. **§64 monthly token refresh:** ✅ `monthlyTokenRefresh` scheduled Pub/Sub function (`0 0 1 * *` UTC) in [functions/src/index.ts](../functions/src/index.ts) — resets Pro users to 200 tokens, bumps renewal, emits log + notification (**deploy-time:** activates on `firebase deploy`, Blaze plan). **§68 monitoring:** ✅ admin System Monitoring tab (see §41–50 row). **⛔ §67 email automation:** seam documented (`// EMAIL SEAM` at `createNotification`; runbook in [docs/OPERATIONS.md](./OPERATIONS.md)) — needs a provider, deferred. |
...
| §79–90 | Phases 2–4, vision | 🟡 (foundation built) | **Phase 6 in progress.** The Master Wiring **spine is complete** (Phase 6.0): tier ladder (`free|pro|team|agency|enterprise`), ownership/visibility model on analyses+reports with V1 back-compat, scope-context + switcher, centralized `can()` permission engine, unified scope-aware history/reports, and the first-ever **server-enforced** [firestore.rules](../firestore.rules) + [firebase.json](../firebase.json). **⛔ remaining:** Team (6.1) / Agency (6.2) / Enterprise (6.3) layer features + their Cloud Functions, simulated tier billing flows, white-label/marketplace. See [ROADMAP.md](./ROADMAP.md) Phase 6. |
...
| 4 | Pro token amount | **200 tokens/mo** — fixed in all code and UI copy. |
