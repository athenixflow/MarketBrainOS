  ...
  return (
    <header className="sticky top-0 z-40 bg-[#0B0B0B]/90 backdrop-blur-xl border-b border-gray-900/50">
      <div className="max-w-7xl mx-auto px-6 md:px-12 h-16 flex items-center justify-between">
        <Link to="/" className="flex items-center gap-4 py-2 -my-2" onClick={() => setOpen(false)}>
        ...
        <div className="hidden md:flex items-center gap-6">
          <Link to="/auth" className="text-[11px] font-bold uppercase tracking-widest text-gray-400 hover:text-white transition-colors py-4 -my-4">Sign In</Link>
        ...
        {/* Mobile toggle */}
        <button onClick={() => setOpen((o) => !o)} className="md:hidden text-gray-300 p-3 -mr-3" aria-label="Menu">
      ...
      {/* Mobile menu */}
      {open && (
        <div className="md:hidden border-t border-gray-900/50 px-6 py-4 space-y-1 animate-in fade-in slide-in-from-top-2 duration-200">
          {LINKS.map((l) => (
            <Link key={l.to} to={l.to} onClick={() => setOpen(false)} className="block py-4 text-xs font-bold uppercase tracking-widest text-gray-400 hover:text-white transition-colors">
              {l.label}
            </Link>
          ))}
          <div className="pt-4 flex items-center gap-4 border-t border-gray-900/50">
            <Link to="/auth" onClick={() => setOpen(false)} className="text-xs font-bold uppercase tracking-widest text-gray-400 py-3.5 -my-1">Sign In</Link>
