    ...
    <div className="space-y-8">
      {canManage && (
        <Card title={editingUid ? 'Edit member' : 'Add a member'}>
          <div className="flex flex-wrap gap-8 mb-6">
            <div><p className="text-[10px] font-black uppercase tracking-widest text-gray-400">Enterprise pool</p><p className="text-lg font-black text-[#0B0B0B] mt-1">{pool.toLocaleString()}</p></div>
          ...
          <p className="mt-5 text-[11px] font-medium text-gray-400 bg-gray-50 border border-gray-100 rounded-xl p-3">
            Note: tool and budget limits are saved with the member and will take effect once enterprise-level analyses ship. Today, members run analyses within their team/agency, where those scopes' limits apply.
          </p>
      ...
      <Card title={`Members (${members.length})`}>
        <div className="space-y-3">
          {members.map(m => {
            const isOwner = m.role === 'enterprise_owner';
              ...
              <div key={m.id} className="flex items-center justify-between gap-4 p-4 bg-gray-50 rounded-xl border border-gray-100 flex-wrap">
                <div className="min-w-0">
                  ...
                  <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mt-1">
                    ...
                    {!isOwner && ` • ${toolCount === 0 ? 'all tools' : `${toolCount} tools`} • budget ${cap > 0 ? cap : 'unlimited'}`}
                  </p>
                </div>
